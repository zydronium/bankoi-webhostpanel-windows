...
<%
	...
	'------------------------------------------------------------------------------------------------------------------------------------------------
		if(RecordType = "MX") then
				...
				record=RecordType
				val=Exch & "."
				query = "select * from tbldnstemplate where host='" & host & "' and recordtype='" & record & "' and resellerid='" & resellerid & "' and mxpref = '" & mx & "'"
				'Response.write query & "<BR>"
...
<%
				...
				query="insert into tbldnstemplate set resellerid='" & resellerid & "', host='" & host & "',recordtype='" & record & "',value='" & val & "',mxpref = '"& mx &"'"
				'Response.write query & "<BR>"
				con.Execute (query)
		End if
